...
def canon_geo(elements,geo,adj_mat,atom_types,bond_mat=None,dup=[]):
    ...
    if len(dup) > 0:
        ...
        if bond_mat is not None:
            return elements,geo,adj_mat,atom_types,bond_mat,N_dup
        else:
            return elements,geo,adj_mat,atom_types,N_dup
    else:
        if bond_mat is not None:
            return elements,geo,adj_mat,atom_types,bond_mat
        else:
...
def find_lewis(elements,adj_mat_0,bonding_pref=[],q_tot=0,fixed_bonds=[],fc_0=None,keep_lone=[],return_pref=False,verbose=False,b_mat_only=False,return_FC=False,octet_opt=True,check_lewis_flag=False):
    ...
    for dummy_counter in range(lewis_total):
        ...
        unsatisfy = [count_t for count_t,te in enumerate(total_electron) if te > 2 and te < 8 and te % 2 == 0 and elements[count_t].lower() in ["c","n","o","f","si","p","s","cl"]]
        ...
        for count_fc in fc_ind:
            ...
            for carbon_ind in nearby_carbon:
                ...
                if len(carbon_nearby) == 3: 
                    fc_hc -= charge*(len([nind for nind in carbon_nearby if elements[nind].lower() == 'c'])*2 + len([nind for nind in carbon_nearby if elements[nind].lower() == 'h']))
    ...
    lewis_criteria = []
    for i in range(len(lewis_lone_electrons)):
        lewis_criteria.append( 10.0 * octet_violations[i] + abs(formal_charges_sums[i]) + 0.1 * sum([ 1 for j in lewis_lone_electrons[i] if j % 2 != 0 ]) + \
                               0.05 * lewis_fc_aro[i] + 0.01 * lewis_fc_all[i] + 0.005 * lewis_fc_en[i] + 0.001 * lewis_fc_pol[i] + 0.0001 * lewis_fc_hc[i] + \
...
def frag_find_lewis(elements,adj_mat_0,bonding_pref=[],fixed_bonds=[],q_tot=0,fc_0=None,keep_lone=[],return_pref=False,return_FC=False,octet_opt=True,check_lewis_flag=False):
    ...
    for i in range(len(elements)):
        ...
        if is_cyano(i,adj_mat_0,elements) is True:
            C_ind = [ count_j for count_j,j in enumerate(adj_mat_0[i]) if j == 1 and elements[count_j] in  ["c","C"] and sum(adj_mat_0[count_j]) == 2 ]
            fixed_atoms = sum([[fb[0],fb[1]] for fb in fixed_bonds],[])
            # if C in N-C-X doesn't involve in fixed_bonds
            if C_ind[0] not in fixed_atoms:
                bonding_pref  = [ j for j in bonding_pref if j[0] != i and j[0] not in C_ind ] # remove bonds involving the cyano atoms from the bonding_pref list
                bonding_pref += [(i,3)]
                bonding_pref += [(C_ind[0],4)]
                bonding_electrons[C_ind[0]] += 2
                ...
                if len(unhappy) > 0:
                    ...
                    try:
                        ...
                        try:
                           ...
                           print("ERROR: something's wrong with find_lewis, check your input structure")
                           quit()
                        break
...
#              N_adj_mat:    new adjacency matrix
#              N_dup:        user supplied lists indexed to the original geometry, now indexed to the new geometry
def mode_geo(m_ind,geo,adj_mat,gens=2,dup=[],bond_mat=None,force_linear=False):
        ...
        #
            ...
            for i in edge_ind:
                for j in edge_ind:
                    N_adj_mat[i,j] = 0
                    N_adj_mat[j,i] = 0
                    if len(bond_mat) > 0 and len(bond_mat) != len(geo):
                        for lb in range(len(bond_mat)):
